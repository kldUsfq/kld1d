...
//variables for becke
int rule_lebedev=7;
vector <long double> totalInt; 
vector <long double> partial_int;
...
bool becke88 (){
	...
	*/
	...
	for ( IJK=0;IJK<npoints;IJK++){
		#pragma omp parallel for private (X1,Y1,Z1,III, KLD)
		for ( III =0;III<IN1;III++){
			...
			long double valuePI = partial_int.at(III);
			long double w_beckeI = w_becke[IJK];
			partial_int.at(III) = valuePI + w_beckeI*KLD;
	...
	for (int III =0;III<IN1;III++){
		...
		resultsFileOS << "   " << X1 << "        " <<  partial_int.at(III) << "\n";
